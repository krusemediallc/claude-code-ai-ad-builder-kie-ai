...
**Applies to:** Veo 3.1, Sora 2, Kling 3.0 (via KIE.ai Veo route and jobs route)
...
**Route:** `POST /api/v1/veo/generate`. When anchoring on a still, pass a hosted URL in `imageUrls` and pick `REFERENCE_2_VIDEO` (requires `model: "veo3_fast"`) or `FIRST_AND_LAST_FRAMES_2_VIDEO` (two URLs).
...
**Route:** `POST /api/v1/jobs/createTask` with `model: "sora-2-text-to-video"` (or `sora-2-pro-text-to-video` / `sora-2-image-to-video`). For image-anchored runs, pass a hosted URL in `input.image_input`.
...
**Route:** `POST /api/v1/jobs/createTask` with `model: "kling-3"`. Hosted reference URLs go in `input.image_input`.
...
- [ ] **Aspect ratio:** 9:16 specified (`aspect_ratio: "9:16"` in the request)
