# Seedance 2 — model guide (on KIE.ai)

**KIE endpoint:** `POST https://api.kie.ai/api/v1/jobs/createTask` with `"model": "seedance-2"`
**Auth:** `Authorization: Bearer $KIE_API_KEY`
**Polling:** `GET https://api.kie.ai/api/v1/jobs/recordInfo?taskId=<taskId>`
**Status signals:** `state` — one of `waiting` / `queuing` / `generating` / `success` / `fail`
**Results:** parsed from `data.resultJson` (a JSON-encoded string containing result URLs)

> **Verify the exact Seedance model string on the [KIE marketplace page](https://kie.ai/market)
> before firing.** The value used here, `seedance-2`, is the likely string but should be
> confirmed and recorded in `MASTER_CONTEXT.md`. Also verify which `input.*` fields the
> KIE build of Seedance supports — advanced fields (reference videos / reference audios)
> may not be exposed.

## Request body shape (KIE jobs)
...
```json
{
  ...
  "input": {
    "prompt": "<your prompt>",
    "image_input": ["https://your-host.example.com/product.jpg"],
    "aspect_ratio": "9:16",
    "duration": 15,
    "resolution": "720p",
    "output_format": "mp4"
...
The response is `{code, data: {taskId}}`. Poll `recordInfo` until `state: success`
and parse `data.resultJson` (JSON-encoded string) for the video URL(s).

## API parameters

| Field | Required | Value / Range | Notes |
|-------|----------|---------------|-------|
| `model` | yes | `"seedance-2"` | Confirm exact string on the KIE marketplace page. |
| `input.prompt` | yes | string | 100–260 words sweet spot (see below). |
| `input.image_input` | no | array of PUBLIC URL strings (up to 14) | PUBLIC URLs only — KIE has no presigned-upload endpoint. Host the image yourself and pass the URL. Positional index 0 maps to `@(img1)` in the prompt, index 1 to `@(img2)`, etc. |
| `input.aspect_ratio` | no | `"9:16"`, `"16:9"` | Default `9:16` for UGC/social. Verify supported ratios on the marketplace page. |
| `input.duration` | no | 4–15 (integer seconds) | Continuous range. Default to 15 for full-length clips. |
| `input.resolution` | no | `"480p"`, `"720p"` | Default `720p`. |
| `input.output_format` | no | `"mp4"` | Defaults are fine in most cases. |
| `callBackUrl` | no | URL | Optional webhook for completion. Leave empty to poll. |

**Verify on marketplace before relying on them:** reference-video support, reference-audio
support, explicit audio-enable toggle. If unsupported on the KIE build of Seedance, lean
on text-driven dialogue and image references only.

### `@(img1)` reference image mapping

The Seedance prompting templates use `@(img1)` / `@(img2)` / `@(img3)` tokens inline in the
prompt text to reference product images. On KIE:

- Pass the corresponding images via the `input.image_input` array (index 0 = `@(img1)`,
  index 1 = `@(img2)`, etc.).
- **Images must be PUBLIC URLs.** No file upload. If your image is local, host it on a
  reachable endpoint (your own CDN/bucket, an image host, etc.) first.
- **Keep the `@(img1)` tokens in the prompt text.** The model reads them as pointers to
  the positional reference images. If a generation ignores the reference image, try
  removing the token and relying solely on the `image_input` array as a fallback.

Record findings from the first real generation in `MASTER_CONTEXT.md` so future sessions
benefit from the empirical confirmation.

## Seedance 2 platform guide

These rules are specific to how Seedance 2 interprets prompts. Apply them on top of
whichever style template you use.
...
Seedance 2 responds best to this order:
...
Never use in Seedance 2 prompts: `cinematic`, `professional`, `stunning`, `8k`, `studio`, `perfect`.
...
Seedance 2 supports **4–15 seconds** (continuous, not an enum). Use the script word count to auto-select:
...
The repo also has [ugc-selfie-style.md](ugc-selfie-style.md) — a cross-model UGC guide for Sora 2 / Veo 3.1. The Seedance-specific [seedance-2-ugc.md](seedance-2-ugc.md) has a richer 9-layer formula optimized for Seedance 2's strengths. Use the Seedance version when generating with `model: "seedance-2"` on KIE.

## Adaptation checklist (all styles)

Before submitting any Seedance 2 prompt, verify:

- [ ] **Word count** — prompt is between 100–260 words
- [ ] **Duration** — set based on dialogue word count (4–15s) or default 15s for no-dialogue
- [ ] **Aspect ratio** — `9:16` (vertical/social) or `16:9` (landscape)
...
- [ ] **@(img1) included** — if product image URL is provided, referenced in prompt text
...
- [ ] **Image URLs are PUBLIC and reachable** — `curl -I` returns 200 for each `input.image_input` entry
- [ ] **Model string verified** — confirmed on [kie.ai/market](https://kie.ai/market) and recorded in `MASTER_CONTEXT.md`
