...
2. Call `POST /api/v1/jobs/createTask` with:
   ...
   - `input.resolution`: `2K` (good default for character reference work)
...
1. Host the approved hero image at a publicly reachable HTTPS URL (any CDN / signed URL works — KIE does not accept base64 or file uploads; it reads the image over HTTP from the URL you pass).
...
3. Call `POST /api/v1/jobs/createTask` for each with:
   - Same `model`, `input.aspect_ratio`, `input.resolution` as hero
   - `input.image_input`: `["<hero_hosted_url>"]`
...
`01-hero-front.jpg` is always the approved anchor image. The agent should use this as the primary `input.image_input` entry when generating new content with this influencer.

### After saving

- Store the 10 images in the character folder under `references/influencers/` and also in the session outputs directory (`outputs/{date}-{slug}/`) so the run is self-contained.
...
- **Video generation** — pass the hero (or any angle) as a hosted URL in `imageUrls` for Veo 3.1 (`REFERENCE_2_VIDEO` or `FIRST_AND_LAST_FRAMES_2_VIDEO`) or in `input.image_input` for Sora 2 (`sora-2-image-to-video`)
...
When referencing an existing character, load `01-hero-front.jpg` as the primary reference (host it and pass the URL). For maximum consistency, host multiple angles from the folder and add their URLs to `input.image_input` (up to 14 supported by Nano Banana).
...
**Flow:** Generate hero → user approves → generate 9 angles with hero URL as reference → QA → save to folder → done.
