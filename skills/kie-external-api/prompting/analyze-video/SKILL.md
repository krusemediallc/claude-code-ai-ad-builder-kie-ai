...
name: analyze-video
description: >
  Analyze a reference video and reverse-engineer its style into a reusable Seedance 2
...
get that same style back from Seedance 2 on KIE.ai.

**Critical constraint: Seedance 2 has a 15-second maximum per clip.** The reference
...
`skills/kie-external-api/prompting/prompt-library/` that works the same way
...
```bash
bash "skills/kie-external-api/prompting/analyze-video/scripts/extract-frames.sh" "<video_path>" "/tmp/video-analysis" <num_frames>
```
...
Read the existing Seedance 2 prompting templates to understand the gold standard
...
the prompt. Always include the standard Seedance 2 checks from seedance-2.md.]
```

### Key principles for the template

- **Every prompt is 15 seconds.** This is a hard constraint from Seedance 2. The template
...
- **Include a `@(img1)` product image reference** in the template pattern. The `@(img1)`
  token is a Seedance prompt-level convention — it tells the model to look at the first
  reference image. On KIE, the corresponding URLs live in `input.image_input[0]`,
  `input.image_input[1]`, etc. (indexed positionally).
...
- **Follow the Seedance 2 platform guide.** All templates must comply with
...
1. Save the template to `skills/kie-external-api/prompting/prompt-library/seedance-2-<style-name>.md`
...
> before firing. Record the confirmed model string in `MASTER_CONTEXT.md`.** The likely
> value is `seedance-2`.
...
**Reference image handling on KIE:**

- KIE requires PUBLIC URLs for reference images — there is no presigned upload flow.
- Host the image on any reachable URL (your own CDN/bucket, an image host, etc.), then
  pass the URL as a string inside `input.image_input`.
- `@(img1)` in the prompt text maps to `input.image_input[0]`, `@(img2)` to
  `input.image_input[1]`, and so on (positional indexing).
- Up to 14 reference images supported.

**Reference videos / reference audios:** KIE's Seedance offering may not expose
reference-video or reference-audio inputs. Verify on the
[KIE marketplace page](https://kie.ai/market) — if unsupported, fall back to a single
reference image plus descriptive prompt text.

**Before submitting:**

1. Host the product image at a reachable URL and put that URL in `input.image_input`.
2. Decide on aspect ratio (`9:16` for vertical/social, `16:9` for landscape), resolution, duration.
3. Show the credit cost estimate from MASTER_CONTEXT.md and wait for confirmation.
4. Submit via the API. Response is `{code, data: {taskId}}`.
5. Poll `GET https://api.kie.ai/api/v1/jobs/recordInfo?taskId=<taskId>` until `state` is
   `success` or `fail`. Intermediate states: `waiting`, `queuing`, `generating`.
6. On success, parse `data.resultJson` (a JSON-encoded string) to get the video URL.
7. Download to `outputs/` and surface the URL plus the [kie.ai/logs](https://kie.ai/logs) link.
...
1. Host your product image at a reachable URL (no presigned upload on KIE).
2. Submit via `POST https://api.kie.ai/api/v1/jobs/createTask`:
...
3. Poll `GET https://api.kie.ai/api/v1/jobs/recordInfo?taskId=<taskId>` until `state: success`.
4. Parse `data.resultJson` to extract the video URL and download.
...
```
skills/kie-external-api/
├── SKILL.md                              ← main skill (decision tree, execution checklist)
├── reference.md                          ← API routes, job schemas, polling
├── prompting/
...
│   └── prompt-library/
│       ├── seedance-2.md                 ← Seedance 2 model guide (platform rules)
